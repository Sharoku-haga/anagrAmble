...
{
public:
	HighScoreText(unsigned long highScoreGameTime, NumberDrawer* pNumberDrawer, ScoreTextDrawer* pScoreTextDrawer);
...
private:
	...
	{
		HIGH_SCORE_TEXT,			//!< 「HighScore」のテキスト
		COLON_TEXT,					//!< コロンテキスト
	...
	std::vector<int>				m_HighScoreGameTimeNum;		//!< プレイ時間の数字.1単位ごとに収納している 
	unsigned long					m_HighScoreGameTime;		//!< ハイスコアのゲーム時間

	/** ハイスコアを単位ごとに格納する関数 */
	void StoreHighScoreUnit(void);
